...
{ 
  ...
  for (int i=1; i <= denum->GetNbinsX(); i++) {
   ...
   // const double errLo = TEfficiency::ClopperPearson((int)nDenum,(int)nNum,0.683,false);
    ...
    double errBinomial = (1/nDenum) * sqrt( nNum * (1 - effVal) );
    efficHist->SetBinError(i, errBinomial);
    std::cout << i <<" bin "<<nNum <<" "<<nDenum<<" Eff: "<<effVal<<" Error: " <<errBinomial<<" bin Content: "<< efficHist->GetBinContent(i)<<" " <<  std::endl;
